...
\textsf{Ref.~\cite{John-W.-Negele:1998aa}; \cite[Chapters 3,4]{Greiner_1996}; \cite[Chapters 1,2]{Bogoliubov:1980}}\\
...
\begin{enumerate}[label=(\roman*)]
	...
	\item for bosons the \textbf{canonical commutation relations} (CCR) hold:
	\begin{eq}
		[a_i,a_j]=0=[a_i^\dagger,a_j^\dagger] \tcomma [a_i,a_j^\dagger]=\delta_{ij}
	\end{eq}
	while for fermions the \textbf{canonical anticommutation relations} (CAR) hold:
	\begin{eq}
		\{a_i,a_j\}=0=\{a_i^\dagger,a_j^\dagger\}\tcomma\{a_i,a_j^\dagger\}=\delta_{ij}
	\end{eq}
...
Since we want to have a representation of the (covering of the) Poincaré group all the points in the hyperboloid should be weighted by the same weight, then a natural choice for our Hilbert space for one particle is provided by 
\begin{eq}
...
\end{eq}
then CCR are satisfied, $[a_i,a_j^\dagger]=\delta_{ij}$, hence also in the relativistic case it is possible to obtain canonical commutation relation by smearing $a$ and $a^\dagger$ with an orthonormal basis of the Hilbert space.  
...
\end{eq}
even for $\vec x\neq\vec y$. This violates the ``locality'' even in the weak non-relativistic form shown before. In fact, since the concept of ``present'' (or equivalently of ``simultaneity'') is not universal in relativity, the vanishing of the commutator (for observable fields at least) should hold for $x$ and $y$ space-like separated (we denoted space-like separated coordinates $x$ and $y$ by $x\sls y$). 
...
\textsf{Ref.~\cite{John-W.-Negele:1998aa}; \cite[Section 5.2]{Bratteli_1997}}\\
...
This actually happens in practice also in some simple situations such as for \emph{(spin $1/2$) Fermi gas}.\footnote{This example is presented in \cite[Part B, sec. 1.1]{Strocchi:1985aa}.} Let's consider $N$ fermions in a finite volume $V$ and zero temperature $T=0$. In such conditions the density of states is 
\begin{eq}
...
\end{eq}
where we have two particles for each momentum state due to the Fermi statistic. Recall that $k_F$ is the Fermi momentum. Clearly the ground state $\ket{\psi_0}$ corresponding to such configuration $n(\vec k)$ is not a vacuum for the annihilation operators $a(\vec k,s)$ when $\modk\leq k_F$ because $a(\vec k,s)\ket{\psi_0}\neq0$, since $a(\vec k,s)$ just annihilates a particle which certainly exists in the whole ensemble where all configurations for $\vec k\leq k_F$ are occupied. 
...
\textsf{Ref.~\cite{John-W.-Negele:1998aa}; \cite[Chapter 8,9]{Greiner_1996}; \cite[Section 1.9, Chapter 10]{Kleinert_2015}; \cite[Chapter 4]{Bogoliubov:1980}}\\
...
\begin{eq}
	G^{(n)}(x_1,\ldots,x_n):=\bra0\tp[\ophi(x_1)\ldots\ophi(x_n)]\ket0
\end{eq} 
...
\end{eq}
where in the last step we moved all the unitary operators on the right (since they are inside the time ordering the result is the same).
Now since 
\begin{eq}
...
\begin{eq}\label{eq:thermal-average}
	\langle(\bullet)\rangle_{\beta:=\frac1{kT}}
...
\begin{eq}
	G_M^{\,O_1\ldots O_n}(\tau_1,\ldots, \tau_n):=-\langle\tp_\tau[O_1^{H}(\tau_1) \ldots O_n^{H}(\tau_n)]\rangle_\beta
\end{eq}
Now for the analogue of the Gell-Mann Low formula
\begin{eq}
	\langle\tp_\tau[O_1^H(\tau_1)\ldots O_n^H(\tau_n)]\rangle_\beta=
	\frac{\langle\tp_\tau[O_1^{H_0}(\tau_1)\ldots O_n^{H_0}(\tau_n)e^{-\int_0^\beta\de\tau'\,H_I(\tau')}]\rangle_\beta^0}
...
\begin{eq}
	G_M(\omega_s)&=-\frac1Z\sum_{m,n}e^{-\beta\cenergy_n}\frac{e^{i\omega_s\beta+\beta(\cenergy_n-\cenergy_m)}-1}{i\omega_s+\cenergy_n-\cenergy_m}\bra nO_1\ket m\bra mO_2\ket n\\
	&=-\frac1Z\sum_{m,n}\frac{e^{i\omega_s\beta-\beta\cenergy_m}-e^{-\beta\cenergy_n}}{i\omega_s+\cenergy_n-\cenergy_m}\bra nO_1\ket m\bra mO_2\ket n
\end{eq}
...
For bosonic fields the completeness employes the \emph{coherent states}. We just recall that for the harmonic QM oscillator the coherent states $\ket z$, $z\in\C$, are of the form (with $a$ annihilation operator)
...
It is clear that vectors $\big\{\ket\phi\big\}$ are eigenvectors also for the field operator:\footnote{Notice the difference between $\phi_\alpha$ and $\phi_\alpha(\vec x)$: the former is the eigenvalue of $a_\alpha$ on $\ket\phi$, whereas the latter comes from eq.~\eqref{eq:coh_field_exp}.}
\begin{eq}
...
Besides the fields at fixed time $\ophi(\vec x)$ in the Gell-Mann - Low formula or in the Matsubara formalism (to which we now address our effort) there are also the time-evolution operators. Let us consider an infinitesimal one: for $H$ the Hamiltonian in terms of free fields, $\Delta\tau$ infinitesimal ``immaginary time'' interval, the infinitesimal time-evolution operator is 
\begin{eq}
...
\begin{eq}
	\{a_\alpha,a_{\alpha'}\}\ket\phi=\{\phia,\phi_{\alpha'}\}\ket\phi\neq0
\end{eq}
...
\begin{eq}\label{eq:matsu_GF_PI}
	\langle\tp[a_{\alpha_1}(\tau_1)\ldots a_{\alpha_n}a_{\alpha_{n+1}}^\dagger\ldots a_{\alpha_{2n}}]\rangle_\beta
	:= \frac%
...
\begin{eq}
	&\langle\tp[a_{\alpha_1}(\tau_1)\ldots a_{\alpha_n}a_{\alpha_{n+1}}^\dagger\ldots a_{\alpha_{2n}}]\rangle_\beta=\\
	&\qquad=\frac1Z\varepsilon^{\sigma(\pi)}\Tr\big[e^{-\beta H}\tilde a_{\alpha_{\pi(1)}}(\tau_{\pi(1)})\ldots \tilde a_{\alpha_{\pi(2n)}}(\tau_{\pi(2n)})\big]\\
