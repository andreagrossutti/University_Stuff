...
Suppose that we have only one kind of particle excitation in the system, then in the non-relativistic case\footnote{The relativistic one is obtained imposing $q^2=m^2$.} since $A(\omega, \vec q)$ describes the probability density to measure energy $\omega$ in the state $\int e^{-iqx}\ophi(x)\ket0$ we have that in general
