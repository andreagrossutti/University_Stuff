...
In the non-abelian case we need to take also a path ordering $\operatorname P(-)$, which is just the time-ordering for the ``time'' parametrizing the loop, combined with the trace:\todo{Non mi è chiaro qui su cosa somma la traccia.}
...
\begin{theorem}[OS reconstruction theorem for finite temperature]\label{thm:OS-finite-T} Suppose that functions $\{S_n^\beta\}_{n=0}^\infty$ satisfy the following properties: 
