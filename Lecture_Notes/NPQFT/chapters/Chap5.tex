...
The equations of motion in Hamiltonian formalism are\todo{Non $\dpi=-\fder{H}{\phi}$?}
\begin{eq}
...
\end{eq}
Hence we whould solve the differential equation
...
\end{enumerate}
One should think at $\{\phi_i\}_{i\in I}$ as the union of all the classical solutions of equations of motion (vacuum configurations and local minima of the action) together with a set of ``inequivalent'' configurations which interpolate between different classical solutions. The meaning of ``inequivalent'' is specified by the above conditions. For instance, there may be a configuration $\phi_i$ which coincide with $\phi^S_+$ far to the left of $x_0$, coincide with $\phi^S_-$ far to the right of $x_0$ and near $x_0$ behaves approximately as a classical kink. Then using (1) and (2) we see that $R_i(\phi-\phi_i)$ is very close to zero if $\phi$ is close to $\phi_i$, and $e^{Q_i(\phi-\phi_i)}$ is almost zero if $\phi$ strongly deviates from $\phi_i$. The five properties together ensures that there are no other configurations $\phi_j\not\in\consp_i$ which are a slight deformation of $\phi_i$. 
...
\end{eq}
as a Boltzmann weight of the \emph{gas of defects} $\{\Gamma_{i,1},\ldots,\Gamma_{i,N}\}$ appearing in the configurations of $\consp_i$. From such Boltzmann weight, we can define the action $S(\Gamma_{i,1},\ldots,\Gamma_{i,N})$ associated to the gas\todo{Controllare}
\begin{eq}
...
Finally, neglecting contributions coming from fluctuations around the vacuum, we get that in the ``improved semi-classical approximation'' (s.c.) the partition function of the system can be expressed as
\begin{eq}
	Z_{\text{s.c.}}:=\int_\consp\pide\phi\,e^{-S(\phi)}
	&\approx\sum_{i\in I}\lexp{-S(\Gamma_{i,1},\ldots,\Gamma_{i,N})}\\
...
\end{eq}
or equivalently, setting formally $e^{S(\Gamma_1,\ldots,\Gamma_N)}=0$ if $\{\Gamma_1,\ldots,\Gamma_N\}$ does not correspond to any $\consp_i$,\todo{Controllare}
