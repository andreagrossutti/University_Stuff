...
The term ``soliton'' (which comes from ``solitary wave'') was much later introduced to characterize solutions of wave equations that do not disperse and preserve their form during the propagation. Hence in a sense a classical soliton behaves as a particle in spite of being a wave. 
...
Before entering in the discussion of solitons, a brief formal discussion of the spontaneously broken symmetry (SSB) phenomenon is required, since it appears in the first example of soliton that we analyze. 
...
Naively it seems impossible to define a spontaneously broken symmetry, as for each map $\alpha:\obsalg\to\obsalg$ one might think that always exists a map $\tilde\alpha:\purstate\to\purstate$ defined by
\begin{eq}
...
\end{eq} 
which is clearly invariant under rotations. Consider two ground states at $T=0$ which differs for the orientation of the spins, and let's see if these could be related by some allowed transformation or not, if not we would have a spontaneous symmetry breaking. We are allowed to do only physical (in particular, local) transformations, at least by some limiting procedure. Nevertheless we already took the thermodynamic limit, therefore we are not allowed to move all the spins (including those at infinity) at the same time with a single transformation. We are forced to use some limiting procedure, for instance we can consider to rotate all the spins inside a ball of radius $R$ and in the neighbourhoods of a sphere of radius $R$ and smoothly interpolate the rotation angle to 0, finally increasing $R$ to $\infty$. But as $R$ approaches $\infty$ there are infinitely many spins rotated, hence due to the non trivial interacting energy between spins it may happen that such procedure requires an infinite amount of energy. If the explicit computation of the energy actually give us an infinite energy cost (as it does in three dimensions, but not in two), then spontaneous symmetry breaking occured. 
...
\begin{eq}\label{eq:kink-boundary-conditi}
	...
	x\to\pm\infty
\end{eq}
as otherwise $H$ is divergent. Indeed all the terms inside $H$ are strictly positive, hence both the term depending on $\pi$ and the one depending on $\phi$ should vanish identically at infinity. The absolute minima of $H$ are given by the two configurations
\begin{eq}
...
\begin{eq}\label{eq:eom-kink}
	\der{^2}{x^2}\phi={g^2}(\phi^2-v^2)\phi
\end{eq}
where now $\phi$ depends only on $x$. If we think about $x$ as a time coordinate and $\phi$ as the position $q$  of a particle, then eq.~\eqref{eq:eom-kink} become a Newton equation with potential $V'(q)=-\frac{g^2}4(q^2-v^2)^2$:\todo{Controllare}
\begin{eq}
	\der{^2}{x^2}\phi={g^2}(\phi^2-v^2)\phi
	\quad\leftrightarrow\quad
	\ddot q=-\der {V'}q
\end{eq}
Due to energy conservation we get
\begin{eq}
	\half\left(\der\phi x\right)^2-\frac{g^2}4(\phi^2-v^2)^2=\text{const (in $x$)}\overset{\eqref{eq:kink-boundary-conditi}}=0
	\quad\leftrightarrow\quad
	\half\dot q^2+{V'}(q)=\text{const (in $t$)}
\end{eq}
Hence we should solve the differential equation
\begin{eq}\label{eq:diff-eq-kink}
	\der\phi x=\mp\frac g{\sqrt2}(\phi^2-v^2)
\end{eq}
...
\begin{tikzpicture}
  ...
  \draw[->] (0, -2.2) -- (0, 2.5) node[at={(0.7,2.6)}] {$\phi_\pm^S(x)$};
...
There is a more instructive way to prove that $\phi_\pm^S$ are the local minima at $(\mp v,\pm v)$ boundary conditions (b.c.). Let's write the potential $V(\phi):=\frac{g^2}4(\phi^2-v^2)^2$ in terms of a function $\suppot(\phi)$ called \emph{superpotential} (since it is largely used in supersymmetric theories)
\begin{eq}
	V(\phi)=\half\left(\der\suppot\phi\right)^2
\end{eq}
which implies that $\der\suppot\phi=\frac g{\sqrt2}(\phi^2-v^2)$ and then
\begin{eq}
...
\begin{eq}\label{eq:Ham-kink-suppot}
	...
	\quad\text{for b.c. }(\mp v,\pm v)
\end{eq}
Now notice that the last term gives
\begin{eq}
	&\int\de x\,\der\phi x\der\suppot\phi=\suppot(\phi(+\infty))-\suppot(\phi(-\infty))
	=\pm\Delta\suppot
	\quad\text{for b.c. }(\mp v,\pm v)
\end{eq}
with
...
\begin{eq}\label{eq:Bogom-bound-cond}
	\der\phi x=\mp\der\suppot\phi\quad\text{for b.c. }(\mp v,\pm v)
\end{eq}
which coincides exactly with eq.~\eqref{eq:diff-eq-kink}. Hence the solutions $\phi_\pm^S$ are local minima for which the energy coincides with the Bogomol'nyi bound, notice also that such solutions are the ones with minimal energy given the b.c. $(\mp v,\pm v)$. 
...
\begin{eq}\label{eq:kink-classic-energy-density}
	...
	\overset{\eqref{eq:diff-eq-kink}}=2\cdot\half\left(\der{\phi_\pm^S}x\right)^2=\left(\der{\phi_\pm^S}x\right)^2
\end{eq}
...
\begin{eq}
	H_{\leq2}=\int\de x\,\left[\frac12\opi+\frac12\ochi\left(-\der{^2}{x^2}+2(gv)^2\right)\ochi\right]
\end{eq}
($H_{\leq2}$ is obtained from $H$ neglecting $\polyn(\ochi)$). One can then expand $\ochi$ in terms of the complete set of solutions $\{\chi_k(x)=e^{ikx}\}$ of the differential equation
\begin{eq}
	\left(-\der{^2}{x^2}+2(gv)^2\right)\chi_k=\omega^2_k\,\chi_k
\end{eq}
with eigenvalues
\begin{eq}
	\omega^2_k=k^2+2(gv)^2
\end{eq}
so that one can write
\begin{eq}
	\ochi(x)=\sum_k\op a_k\,\chi_k(x)
\end{eq}
From the quadratic terms of $H$ we see that $\ochi$ is a massive field with (bare) mass $m=\sqrt2gv$, hence we recover the usual dispersion relation $\omega_k=\sqrt{k^2+m^2}$. Self-interaction are given by $\polyn(\ochi)$.
...
\begin{eq}
	H_2=\sum_{n\in\Z}\bigg(\frac{\dot{\op a}_n^2}2+\frac{\omega_n^2}2\op a_n^2\bigg)
\end{eq}
...
\begin{eq}\label{eq:quadr-ham-quantum-kink}
	...
	&=M_S^\tcl+\half M_S^\tcl \,\dot{\op x}\hspace{0cm}^2_0+\sum_{n\in\Z^*}\bigg(\frac{\dot{\op a}_n^2}2+\frac{\omega_n^2}2\op a_n^2\bigg)\\
	&=M_S^\tcl+\frac{\op p_0^2}{2M_S^\tcl}+\sum_{n\in\Z^*}\bigg(\frac{\dot{\op a}_n^2}2+\frac{\omega_n^2}2\op a_n^2\bigg)
\end{eq}
...
\begin{eq}
	 M_S^\tcl+\frac{\op p_0^2}{2M_S^\tcl} \quad\longrightarrow\quad \sqrt{\smash[b]{M_S^{\tcl\,2}+\op p_0^2}}
\end{eq}
...
\begin{eq}
	\spec\left(\sum_{n\in\Z^*}\frac{\dot{\op a}_n^2}2+\frac{\omega_n^2}2\op a_n^2\right)=\sum_{n\in\Z^*}\left(N_n+\half\right)\omega_n
	\twith N_n\in\N
\end{eq}
...
\begin{eq}\label{eq:kink-quantum-mass-renorm}
 	M_S^\tq=M_S^\tcl+\sum_{n\in\Z^*} \Big(\frac{\omega_n}2-\frac{\omega_{\tvac,n}}2\Big)
\end{eq}
...
\begin{eq}
	...
	\tso
	\tilde p_n=p_n+\frac{\delta(\tilde p_n)}{L}\overset{L\gg1}\approx p_n+\frac{\delta(p_n)}{L}
\end{eq}
Then eq.~\eqref{eq:kink-quantum-mass-renorm} reads (with the convention to discard the finite difference due to the eigenvalue  $\omega_1$, as we are interested in the divergent terms)
\begin{eq}
...
\begin{eq}
	\sum_{n\in\Z^*}f(p_n)\quad\longmapsto\quad\int_{-\infty}^\infty\frac{\de p\,L}\pi f(p)
\end{eq}
and then we get
\begin{eq}
...
\begin{eq}
	 M_S^\tq-M_S^\tcl&\approx-\int_0^\infty\frac{\de p}{\pi}\sqrt{p^2+m^2}\,\der{\delta(p)}p
...
\begin{eq}
	\delta\cenergy=\half\delta m^2(\phi_{\text{kink}}^2-\phi_{\text{vac}}^2)=\half\delta m^2v^2\left(\tanh^2\frac{gv(x-x_0)}{\sqrt2}-1\right)
\end{eq}
which, by integration, give the one-loop correction to the mass of the kink
\begin{eq}\label{eq:kink-div-mass-inter}
	\delta M_S&=\half\delta m^2\frac{m^2}{2g^2}\int_{-\infty}^{+\infty}\de x\,\left(\tanh^2\frac m2(x-x_0)-1\right)\\
...
\end{eq}
which is exactly the factor needed to cancel the divergence in the bare mass of $\chi$ in eq.~\eqref{eq:kink-div-bare-mass}. Therefore, at least at first order, (the divergent part of) the correction due to the kinetic energy of fluctuations (eq.~\eqref{eq:kink-div-bare-mass}) cancels exactly (the divergent part of) the correction coming from the renormalization of the bare mass (eq.~\eqref{eq:kink-div-mass-inter}) of $\chi$, i.e. the counterterm needed for the renormalization of the mass of $\chi$ makes finite also the mass of the kink (perturbatively).
...
\end{eq}
A complete treatment including higher order terms in the Lagrangian formalism gives the so called \emph{Nambu-Goto} or \emph{Polyakov actions}. Moreover, if a domain world is coupled to gravity, it ``antigravitates'', that is, it is repulsive for both other walls or for massive particles. 
...
A kink interpolates between the two structures and in terms of the displacement of the double bond it has exactly the form of the kink of $\phi^4$ in the lattice formulation. An example of such situation is shown in fig.~\ref{fig:kink-polyacetylene}.
...
\end{eq}
usually called \emph{covariance}, then assuming translational invariance we can perform the Fourier transform of $C$, $\tilde C(k)$, and then there is a theorem \todo{Come si chiama il teorema? (O dove si può trovare?)} proving that typical configurations of $\phi$ satisfy with probability 1 the estimate
...
\begin{enumerate}[label=(\arabic*), start=0]
	...
	\item $\bigcup_i\consp_i=\consp$ is the configuration space of $\phi$;
	\item $\consp_i\cap\consp_j=\emptyset$ for $i\neq j$.
\end{enumerate}
One should think at $\phi_i$ as a configuration which is given by local approximate solutions of the equations of motion (vacuum configurations and local minima of the action) together with a smooth interpolation between different solutions.  For instance for $+$ b.c., there may be a configuration $\phi_i$ which approximately coincides with $\phi_+$ far to the left of some point $x_-$, behaves approximately $\phi^S_-$ near $x_-$, approximately coincides with $\phi_-$ far to the right of $x_+$ and far left to a point $x_+\gg x_-$, behaves approximately like $\phi^S_+$ near $x_+$ and approximately coincides with $\phi_+$ far to the right of $x_+$.  The set $\{\phi_i\}_{i\in I}$ is the union of a complete set, in the sense of property (3),(4), of such ``inequivalent'' local approximate classical solutions of equations of motion. Then using (1) and (2) we see that $R_i(\phi-\phi_i)$ is very close to zero if $\phi$ is close to $\phi_i$, and $e^{Q_i(\phi-\phi_i)}$ is almost zero if $\phi$ strongly deviates from $\phi_i$. The five properties together ensures that there are no other configurations $\phi_j\not\in\consp_i$ which are a slight deformation of $\phi_i$. 
...
If the Euclidean space-time minus the support of the defect $\Gamma$ is contractible to the Euclidean space-time minus a $n$-dimensional manifold, then we call the defect a $n$-dimensional defect. In particular we can have a point defect, a line defect, etc. In mathematical language, $\Gamma$ is a point/line/surface/\ldots/$n$-dimensional defect if its support is the tubular neighbourhood of a point/line/surface/\ldots/$n$-dimensional manifold. 
...
\end{tikzpicture}
\caption{The defect $\Gamma$ is represented by the blue thick line. The black line correspond to the locus of the defect, i.e. where $\phi=0$. In the white regions the configuration is close to the vacuum, $|\phi|\approx v$.}
\label{fig:defect}
\end{figure}

If the vacuum expectation value is positive, $\langle\phi\rangle>0$, which can be achieved by imposing $\phi_+$ boundary conditions at $\infty$, the situation is the one represented in fig.~\ref{fig:defects-positive-boundaries-conditions}. Notice that for such boundary conditions it is impossible to have defects reaching $\infty$. 
...
\end{tikzpicture}
\caption{Support of defects (blue lines) and locus of such defects (black lines) in presence of $\langle\phi\rangle>0$ boundary conditions, in the finite volume regularization.}
...
\begin{eq}
	\int_{\consp_i}\pide\phi\,e^{-S(\phi)} \overset{\text{s.c.}}\approx \int_{\consp_i-\phi_i}\pide\chi_i\,e^{-S(\phi_i)-Q_i(\chi_i)}
\end{eq}
as a Boltzmann weight of the \emph{gas of defects} $\{\Gamma_{i,1},\ldots,\Gamma_{i,N}\}$ appearing in the configurations of $\consp_i$, with $\overset{\text{s.c.}}\approx$ denoting the (improved) semi-classical approximation discussed above. From such Boltzmann weight, we can define the action $S(\Gamma_{i,1},\ldots,\Gamma_{i,N})$ associated to the gas
\begin{eq}
	e^{\displaystyle -S(\Gamma_{i,1},\ldots,\Gamma_{i,N})}=\int_{\consp_i}\pide\phi\,e^{-S(\phi)}\overset{\text{s.c.}}\approx \int_{\consp_i-\phi_i}\pide\chi_i\,e^{-S(\phi_i)-Q_i(\chi_i)}
\end{eq}
Notice that the same defect $\Gamma$ can be associated to different sets $\consp_j$, as in general classical configurations may interpolate between several local minima of the action, but each configuration of defects $\{\Gamma_1,\ldots,\Gamma_N\}$ is uniquely associated to some $\consp_i$, for the same reasons which lead us to eq.~\eqref{eq:improv-semiclass-approx}. 

We can now define the interaction between defects in the configuration $\{\Gamma_1,\ldots,\Gamma_N\}$ as
\begin{eq}
	V(\Gamma_1,\ldots,\Gamma_N):=S(\Gamma_1,\ldots,\Gamma_N)-\sum_{k=1}^NS(\Gamma_k)
\end{eq}
One should notice that if the defects are separated enough from each others, one can consider approximately $V(\Gamma_1,\ldots,\Gamma_N)=0$ and the defects not interacting. 

Finally, ``neglecting'' (actually more precisely factoring them out) contributions coming from fluctuations around the vacuum, we get a representation of the partition function in terms of defects which can be treated in the ``improved semi-classical approximation'' (s.c.):
\begin{eq}
	Z_{\text{s.c.}}\approx\sum_{i\in I}\lexp{-S(\Gamma_{i,1},\ldots,\Gamma_{i,N})}
	=\sum_{i\in I}\underbrace{\prod_{k=1}^N\lexp{-S(\Gamma_{i,k})}}_{\text{Non-interacting}\atop\text{gas of defects}}\,\underbrace{\vphantom{\prod_{k}}\lexp{-V(\Gamma_{i,1},\ldots,\Gamma_{i,N})}}_{\text{Interactions}\atop\text{among defects}}
\end{eq}
or equivalently, setting formally $e^{-S(\Gamma_1,\ldots,\Gamma_N)}=0$ if $\{\Gamma_1,\ldots,\Gamma_N\}$ does not correspond to any $\consp_i$,
\begin{eq}\label{eq:Zsc}
	Z_{\text{s.c.}}\approx\sum_{N=0}^\infty\sum_{\{\Gamma_1,\ldots,\Gamma_N\}}\lexp{-S(\Gamma_{1},\ldots,\Gamma_{N})}
\end{eq}
where the sum over $\{\Gamma_1,\ldots,\Gamma_N\}$ considers all possible combinations of $N$ defects.
From eq.~\eqref{eq:Zsc} we see that $Z_{\text{s.c.}}$ can be interpreted as a partition function of the interacting gas of defects, which by construction have closed locus.
...
\end{tikzpicture}
\caption{The sites of the lattice are denoted by $+$ and $-$ depending on whether the field has positive or negative value on them. The blue lines represent defects which are supported in the dual lattice and are constructed according to fig.~\ref{fig:defect-lattice-1}.}
...
\end{eq}
We can also write\footnote{Let us write a function of a definite positive operator $A$ as the linear combination $f(A)=\sum_nf(\lambda_n)\ket{\lambda_n}\bra{\lambda_n}$, where $\{\lambda_n\}$ is the set of positive eigenvalues of $A$ with eigenvectors $\ket{\lambda_n}$. 
For $a>0$ we have 
...
\begin{eq}
	(-\Delta+m^2)^{-1}(x,y)=\int_0^\infty\de s\,\left(\lexp{-s(-\Delta+m^2)}\right)(x,y)=\int_0^\infty\de s\,\lexp{-sm^2}\lexp{s\Delta }(x,y)
\end{eq}
...
\begin{eq}
	\left(e^{is\Delta}\right)(x,y)=\!\!\!\underset{\text{\footnotesize $q(0)=x$}\atop \text{\footnotesize $q(s)=y$}}\int\!\!\!\pide q(t)\,e^{\displaystyle \,  i\text{\footnotesize$\int_0^s$}\de t\,\dot q^2(t)/4}
\end{eq}
which implies
\begin{eq}
	\left(e^{s\Delta}\right)(x,y)=\!\!\!\underset{\text{\footnotesize $q(0)=x$}\atop \text{\footnotesize $q(s)=y$}}\int\!\!\!\pide q(t)\,e^{\displaystyle \,  -\text{\footnotesize$\int_0^s$}\de t\,\dot q^2(t)/4}
\end{eq}
so that finally
\begin{eq}
	\langle\phi(x)\phi(y)\rangle=\int_0^\infty\!\de s\,\!\!\!\underset{\text{\footnotesize $q(0)=x$}\atop \text{\footnotesize $q(s)=y$}}\int\!\!\!\pide q(t)\,\underbrace{\vphantom{\int}e^{\displaystyle \, -sm^2 -\text{\footnotesize$\int_0^s$}\de t\,\dot q^2(t)/4}}_{\text{Boltzmann weight}}
\end{eq}
that is, we expressed the two points function as a weighted ``sum'' over trajectories with boundary points $x$ and $y$.

Analogously, for the partition function\footnote{Analogously to what we have done in eq.~\eqref{eq:integral-form-inverse-operator}, for $A$ definite positive, $a>0$, we have
\begin{eq}
	\der{}a\int_0^\infty\frac{\de s}se^{-sa}=-\int_0^\infty e^{-sa}=-\frac1a
	\quad\Rightarrow\quad
	\int_0^\infty\frac{\de s}se^{-sa}=-\log a+c
	\quad\Rightarrow\quad
	\log A=-\int_0^\infty\frac{\de s}se^{-sA}-c
\end{eq}
...
\begin{eq}
	...
	&=\exp\left[\int\de^{d+1}x\int_0^\infty\frac{\de s}s\,e^{-sm^2}\smash{\!\!\!\underset{\text{\footnotesize $q(0)=q(s)=x$}}\int\!\!\!\!\pide q(t)\,e^{\displaystyle \,  -\text{\footnotesize$\int_0^s$}\de t\,\dot q(t)^2/4}}\ \right]\\[1em]
	&=\sum_{n=0}^\infty\frac1{n!}\left[\int\de^{d+1}x\int_0^\infty\frac{\de s}s\,\smash{\!\!\!\underset{\text{\footnotesize $q(0)=q(s)=x$}}\int\!\!\!\!\pide q(t)\,\underbrace{\vphantom{\int}e^{\displaystyle \,  -sm^2-\text{\footnotesize$\int_0^s$}\de t\,\dot q(t)^2/4}}_{\text{Boltzmann weight}}}\ \right]^n\\[1em]
\end{eq}
...
\end{eq}
where $\gamma$ is a regular curve connecting $x$ and $y$ and $(\ldots)_{\text{ren}}$ denotes a suitable renormalization needed to remove the cutoffs we introduced to avoid Haag theorem. For higher (even) number of insertion points different curves connecting pairs of points should be considered, and an appropriate covariant derivative should be used.\footnote{It is not needed to consider different possible connections between points, as in the path integral all the possible choices are taken into account considering also all possible closed defects.}
...
\end{eq}
provided that $B$ is compact. This implies that the correlator eq.~\eqref{eq:two-points-function-kink} is invariant under $\xi_B$. 
...
If different open defects are present (several insertion points in the correlator) then the interaction between these defects depend only on the insertion points, and not on how close are the curves chosen to build the covariant derivative. 
...
\end{eq}
so that the reconstructed fields satisfy a new commutation relation called \emph{dual algebra}\footnote{Such new algebra appeared for the first time in \cite{Frohlich1976}.} (writing $x^1$ instead of $(0,x^1)$, same for $y_1$)\todo{Aggiungere commento su $\epsilon$ introdotto da OS reconstruction nel tempo immaginario.}
...
so that in the first case $U(\omega_y\vert\gamma)=-1$ whereas in the second one $U(\omega_y\vert\gamma)=1$. In terms of the field $\phi_\gamma$ we get $\phi_\gamma(y)=-\phi(y)$ if the boundary point of $\gamma$ is $x^\epsilon$, and $\phi_\gamma(y)=\phi(y)$ if the boundary point is $x^{-\epsilon}$. Hence in the limit $\epsilon\to0$ we get eq.~\eqref{eq:schwinger-dual-algebra-kink} for $x^1>y^1$ (consider for instance the simplest case of eq.~\eqref{eq:4-point-kink-meson}). 

Instead, in the case $x^1<y^1$, the situation in the following: 
...
In this case $U(\omega_y\vert\gamma)=U(\omega_y\vert\gamma)=1$, so $\phi_\gamma(y)=\phi(y)$ for both $x^\epsilon$ and $x^{-\epsilon}$ and we get eq.~\eqref{eq:schwinger-dual-algebra-kink} for $x^1<y^1$.

\skipline

Notice that eq.~\eqref{eq:dual-algebra} give a new type of commutation relations, different respect to both bosonic and fermionic ones, which depend on the space coordinates. Such statistics is the \emph{braid statistics} in 1 dimension. Its concrete realization differs from that in 2 dimensions since there the phase introduced in the commutation relation depends on whether the particles are interchanged in the clockwise or in the anticlockwise direction, whereas in 1 dimension it depends on whether the order of the quantum fields agrees with the order of their position in $\R$. However the same abstract group underlies these statistics, the braid groups replacing the permutation groups.
...
\end{eq}
Chosing  $\gamma$ as in the figure below the numerator in the previous expression can be represented by 
%
\begin{figure}[H]
\centering
\tikzstyle{arrow} = [thick,->,>=stealth]
...
With positive boundaries conditions the reconstruction theorem proves that the Hilbert space is decomposed in a vacuum sector  and a kink sector
\begin{eq}
	\hs=\hs_+\oplus\hs_S
	\twith
	\ket\Omega\in\hs_+
...
In the ``unbroken'' phase with $\Z_2$-global symmetry, it turns out that $\hs$ cannot be decomposed in the direct sum of two subspaces. Let us show intuitively why. If the $\Z_2$ symmetry is restored, then all typical configurations are closed to the vacuum $\phi_0$, which now vanishes.\footnote{Analogously, in the Ising model the $\Z_2$ symmetry is restored in the paramagnetic phase, where $\langle S_i\rangle=0$.} Therefore it is much easier to find configurations where $\phi$ vanishes. Hence there are much more defects respect to the broken-phase case, and typically the net of defects (the collection of regions where $\phi$ vanishes) reaches the boundary $\infty$ of the spacetime (see the last section). Then an additional line defect $\gamma$ from $z$ to $\infty$ cannot affect the correlation function, so that $\lim_ {z \rightarrow \infty} \bra\Omega \op s(x) \op s(z)\ket{\Omega}\neq 0$.
But then by the cluster property the kink position sent to infinity is incompatible with $\bra{\Omega}\op s(x)\ket{\Omega}=0$. Hence the Hilbert space $\hs$ has no superselection kink sectors, $\hs\equiv\hs_0$, and the phase in which the net of defects reaches the infinity corresponds to the unbroken $\Z_2$-symmetry.  

The configuration, which appears in the unbroken phase, in which the defects almost covers the whole space time, is said \emph{condensation of kinks} and it will be briefly discussed in section~\ref{sec:condensation-defects}.
