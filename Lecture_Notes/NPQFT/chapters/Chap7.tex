...
\begin{eq}
	\lexp{i\op\theta(\vec x)}\quad\to\quad \lexp{i(\op\theta(\vec x)+\Lambda(\vec x))}
\end{eq}
...
\begin{eq}
	\lexp{i\int\de^3\vec y\, E_i^{\vec x}(\vec y)\op A^i(\vec y)}
\end{eq}
...
\begin{eq}
	\lexp{i\int\de^3\vec y\, E_i^{\vec x}(\vec y)\op A^i(\vec y)}
	\quad\to\quad
	&\lexp{i\int\de^3\vec y\, E_i^{\vec x}(\vec y)(\op A^i(\vec y)+\partial_i\Lambda(\vec y))}=\\
	&\quad=\lexp{i\int\de^3\vec y\,E_i^{\vec x}(\vec y)\op A_i(\vec y)}\lexp{-i\int\de^3\vec y\,\partial^i E_i^{\vec x}(\vec y)\Lambda(\vec y)}=\\
	&\quad=\lexp{i\int\de^3\vec y\,E_i^{\vec x}(\vec y)\op A_i(\vec y)}\lexp{-i\Lambda(\vec x)}
\end{eq}
so that 
\begin{eq}\label{eq:Dirac-recipe-quant}
	\lexp{i\op\theta(\vec x)}\lexp{i\int\de^3\vec y\, E_i^{\vec x}(\vec y)\op A^i(\vec y)}
\end{eq}
...
\begin{eq}
	\lexp{i\theta(x)}\lexp{i\int\de^4y\,E_\mu^x(y)A^\mu(y)}
\end{eq}
...
\begin{eq}\label{eq:exp-value-monopole-duality}
	\big\langle \lexp{i\theta(x)}\lexp{i\int\de^4z\,E_\mu^x(z)A^\mu(z)}\lexp{-i\theta(y)}\lexp{-i\int\de^4w\,E_\mu^y(w)A^\mu(w)}\big\rangle_\lambda
\end{eq}
...
\begin{eq}
	&\langle \lexp{i\theta(x)}\lexp{i\int\de^4z\,E_\mu^x(z)A^\mu(z)}\lexp{-i\theta(y)}\lexp{-i\int\de^4w\,E_\mu^y(w)A^\mu(w)}\rangle_\lambda=\\
	&\qquad=\langle \lexp{i\int\de^4w\,(E_\mu^x(w)-E_\mu^y(w))A^\mu(w)}\lexp{i\int\de^4w\,j_\mu^{xy}(w)(\partial^\mu\Lambda+2\pi n^\mu)(w)}\rangle_\lambda
\end{eq}
Note that 
\begin{eq}
...
\begin{eq}
	&\langle \lexp{i\theta(x)}\lexp{i\int\,E_\mu^xA^\mu}\lexp{-i\theta(y)}\lexp{-i\int\,E_\mu^yA^\mu}\rangle_\lambda=\\
	&\qquad=\frac1{Z_\lambda}\int\pide A_\mu\int\pide\Lambda\sum_{n_\mu}\,\lexp{i\int\,(E_\mu^x-E_\mu^y)A^\mu}\lexp{i\int\,j_\mu^{xy}\partial^\mu\Lambda}\lexp{-\frac1{2e^2}\int(\partial_{[\mu}A_{\nu]})^2}\lexp{-\frac\lambda2\int(\partial_\mu\Lambda-A_\mu+2\pi n_\mu)^2}\delta(\partial^\mu A_\mu)\\
	&\qquad\overset{\mathclap{\eqref{eq:Poisson-resummation-cont}}}=\,\frac1{Z_\lambda}\int\pide A_\mu\int\pide\Lambda\sum_{j_\mu}\,\lexp{i\int\,(E_\mu^x-E_\mu^y)A^\mu}\lexp{i\int\,j_\mu^{xy}\partial^\mu\Lambda}\lexp{-\frac1{2e^2}\int(\partial_{[\mu}A_{\nu]})^2}\lexp{-\frac1{2\lambda}\int j_\mu^2}\lexp{i\int(\partial_\mu\Lambda-A_\mu)j^\mu}\delta(\partial^\mu A_\mu)\\
	&\qquad\overset{\mathclap{\eqref{eq:int-Lambda-Poisson}}}=\ \frac1{Z_\lambda}\int\pide A_\mu\sum_{j_\mu}\,\lexp{i\int\,(E_\mu^x-E_\mu^y)A^\mu}\lexp{-\frac1{2e^2}\int(\partial_{[\mu}A_{\nu]})^2}\lexp{-\frac1{2\lambda}\int j_\mu^2}\lexp{-i\int A_\mu j^\mu}\delta(\partial^\mu j_\mu+\partial^\mu j_\mu^{xy})\delta(\partial^\mu A_\mu)\\
\end{eq}
...
\begin{eq}\label{eq:monop-correlat-lambda-infty}
	&\langle \lexp{i\theta(x)}\lexp{i\int\de^4z\,E_\mu^x(z)A^\mu(z)}\lexp{-i\theta(y)}\lexp{-i\int\de^4w\,E_\mu^y(w)A^\mu(w)}\rangle_\infty=\\
...
\begin{eq}
	&\langle \lexp{i\theta(x)}\lexp{i\int\de^4z\,E_\mu^x(z)A^\mu(z)}\lexp{-i\theta(y)}\lexp{-ie\int\de^4w\,E_\mu^y(w)A^\mu(w)}\rangle_\infty=\\
	&\qquad=\frac
